...
void easyEncode(const std::uint8_t * uncompressed, int uncompressedSizeBytes, std::uint8_t ** compressed, int * compressedSizeBytes, int * compressedSizeBits);
...
int easyDecode(const std::uint8_t * compressed, int compressedSizeBytes, int compressedSizeBits, std::uint8_t * uncompressed, int uncompressedSizeBytes);
...
BitStreamReader::BitStreamReader(const BitStreamWriter & bitStreamWriter) : stream(bitStreamWriter.getBitStream())
, sizeInBytes(bitStreamWriter.getByteCount()), sizeInBits(bitStreamWriter.getBitCount()){
    reset();
}

BitStreamReader::BitStreamReader(const std::uint8_t * bitStream, const int byteCount, const int bitCount) : stream(bitStream), sizeInBytes(byteCount), sizeInBits(bitCount){
    reset();
}
...
Encoder::Encoder(const std::uint8_t * data, const int dataSizeBytes, const bool prependTreeToBitStream) : treeRoot(nullptr), treePrefixBits(0){
...
Decoder::Decoder(const BitStreamWriter & encodedBitStream) : bitStream(encodedBitStream){
    readPrefixData();
}

Decoder::Decoder(const std::uint8_t * encodedData, const int encodedSizeBytes, const int encodedSizeBits) : bitStream(encodedData, encodedSizeBytes, encodedSizeBits){
    readPrefixData();
}
...
int easyDecode(const std::uint8_t * compressed, const int compressedSizeBytes, const int compressedSizeBits, std::uint8_t * uncompressed, const int uncompressedSizeBytes){
